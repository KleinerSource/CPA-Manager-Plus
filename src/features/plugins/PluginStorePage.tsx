...
type InstallPhase = 'confirm' | 'running' | 'success' | 'error' | 'cancelled';
...
interface InstallDialogState {
  ...
  target: string;
  phase: InstallPhase;
  progress: PluginStoreInstallProgressEvent | null;
  errorMessage: string;
}
...
export function PluginStorePage() {
  ...
  const closeInstallDialog = useCallback(() => {
    if (installDialog?.phase === 'running') {
      installAbortRef.current?.abort();
      return;
    }
  ...
  const startInstall = useCallback(async () => {
    if (!installDialog || installDialog.phase === 'running') return;
    const { entry, isUpdate } = installDialog;
    ...
    try {
      const result = await pluginStoreApi.installStream(entry.id, {
        signal: controller.signal,
        onProgress: (event) => {
          setInstallDialog((current) => (current ? { ...current, progress: event } : current));
        },
      });
      setInstallDialog((current) =>
        current ? { ...current, phase: 'success', progress: null, errorMessage: '' } : current
      );
      showNotification(
      ...
      if (isAbortError(err)) {
        setInstallDialog((current) =>
          current ? { ...current, phase: 'cancelled', errorMessage: '' } : current
        );
      } else {
        const message = getErrorMessage(err, t(failedKey));
        setInstallDialog((current) =>
          current ? { ...current, phase: 'error', errorMessage: message } : current
        );
        showNotification(`${t(failedKey)}: ${message}`, 'error');
      }
    } finally {
  ...
  }, [clearConfigCache, installDialog, loadStore, showNotification, t]);

  const installProgress = installDialog?.progress;
  const installPercent =
    installProgress && installProgress.total > 0
      ? Math.min(100, Math.max(0, installProgress.percent))
      : 0;
  const installStage = installProgress?.stage ?? '';
  const downloadArchiveComplete =
    installStage === 'download_archive' &&
    Boolean(installProgress?.total && installProgress.downloaded >= installProgress.total);
  const installingAfterDownload =
    downloadArchiveComplete || ['download_checksum', 'verify', 'write'].includes(installStage);
  const installStageLabel = installingAfterDownload
    ? t('plugin_store.install_stage_installing')
    : installStage
      ? t(`plugin_store.install_stage_${installStage}`, {
          defaultValue: t('plugin_store.install_stage_working'),
        })
      : t('plugin_store.install_stage_waiting');
  const installProgressText =
    installProgress && installProgress.total > 0
      ? t('plugin_store.install_progress_bytes', {
          downloaded: formatBytes(installProgress.downloaded),
          total: formatBytes(installProgress.total),
        })
      : installDialog?.phase === 'running'
        ? t('plugin_store.install_progress_waiting')
        : '';
            ...
            <div className={styles.installDialogActions}>
              {installDialog.phase === 'running' ? (
                <Button variant="secondary" onClick={closeInstallDialog}>
                  {t('plugin_store.cancel_download')}
                </Button>
              ) : installDialog.phase === 'confirm' ? (
                <>
                  <Button variant="secondary" onClick={closeInstallDialog}>
                    {t('common.cancel')}
                  </Button>
                  <Button onClick={() => void startInstall()}>
                    {installDialog.isUpdate ? t('plugin_store.update') : t('plugin_store.install')}
                  </Button>
                </>
              ) : (
                <Button onClick={() => setInstallDialog(null)}>{t('common.close')}</Button>
              )}
            </div>
          ) : null
        }
      >
        {installDialog ? (
          <div className={styles.installDialogBody}>
            <p className={styles.installDialogMessage}>
              {installDialog.isUpdate
                ? t('plugin_store.update_confirm_message', { target: installDialog.target })
                : t('plugin_store.install_confirm_message', { target: installDialog.target })}
            </p>
                ...
                <div className={styles.installProgressHeader}>
                  ...
                  <strong>
                    {installProgress?.total
                      ...
                      : t('plugin_store.install_progress_active')}
                  </strong>
                </div>
                <div
                  className={`${styles.installProgressTrack} ${
                    installProgress?.total ? '' : styles.installProgressTrackIndeterminate
                  }`}
                  role="progressbar"
                  aria-valuemin={0}
                  aria-valuemax={100}
                  aria-valuenow={installProgress?.total ? Math.round(installPercent) : undefined}
                >
                  <span
                    className={styles.installProgressBar}
                    style={{ width: installProgress?.total ? `${installPercent}%` : undefined }}
                  />
                </div>
                <div className={styles.installProgressMeta}>
                  <span>{installProgressText}</span>
                  {installProgress?.assetName ? <span>{installProgress.assetName}</span> : null}
                </div>
              </div>
            ) : null}

            {installDialog.phase === 'success' ? (
              <div className={styles.installStatusSuccess}>
                {installDialog.isUpdate
                  ? t('plugin_store.update_success')
                  : t('plugin_store.install_success')}
              </div>
            ) : null}
            {installDialog.phase === 'cancelled' ? (
              <div className={styles.installStatusInfo}>{t('plugin_store.install_cancelled')}</div>
            ) : null}
            {installDialog.phase === 'error' ? (
              <div className={styles.installStatusError}>
                {t(
                  installDialog.isUpdate
                    ? 'plugin_store.update_failed'
                    : 'plugin_store.install_failed'
                )}
                {installDialog.errorMessage ? `: ${installDialog.errorMessage}` : ''}
              </div>
            ) : null}
          </div>
        ) : null}
