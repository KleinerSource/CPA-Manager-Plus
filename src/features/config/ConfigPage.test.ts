...
import {
  ...
  resolvePanelHostedByUsageServiceFromSession,
} from './ConfigPage';
...
describe('resolvePanelHostedByUsageServiceFromSession', () => {
  it('uses the authenticated session mode before probing Manager Server info', () => {
    expect(resolvePanelHostedByUsageServiceFromSession('external_panel')).toBe(false);
    expect(resolvePanelHostedByUsageServiceFromSession('manager_embedded')).toBe(true);
    expect(resolvePanelHostedByUsageServiceFromSession('')).toBe(null);
  });
});
